...
---
## 狀態特徵工程詳解 (Detailed State Feature Engineering)

### 特徵詞彙表 (Glossary of Features)

#### 全域特徵 (Global Features)
| 特徵 | 中文名稱 | 說明 |
| :--- | :--- | :--- |
| `norm_num_tardy_jobs` | 標準化已延遲作業數 | 當前已延遲的作業數量，除以總作業數進行標準化。 |
| `due_ratio` | 平均交付日期達成率 | 已完成作業的「完成時間 / 交付日期」的平均值，反映整體的延遲趨勢。 |
| `CRO` | 總體工序完成率 | 已完成的總工序數佔所有作業總工序數的比例。 |
| `max_failure_risk` | 最大故障風險 | 系統中所有機器當前故障率 (`lambda`) 的最大值。 |
| `mean_machine_load` | 平均機器負載 | 所有機器負載（時間利用率）的平均值。 |
| `std_machine_load` | 機器負載標準差 | 機器負載的標準差，衡量負載均衡性。 |
| `diff_machine_load` | 機器負載極差 | 最大與最小機器負載的差距。 |
| `norm_total_pdm` | 標準化總PM次數 | 系統執行的總預防性維護次數，經過標準化。 |

#### 各機器的特徵 (Per-Machine Features)
| 特徵 | 中文名稱 | 說明 |
| :--- | :--- | :--- |
| `age` | 虛擬年齡 | 機器自上次維護後，已累積的工作時間。 |
| `lambda` | 當前故障率 | 根據 `age` 和韋伯分佈計算出的瞬時故障機率。 |
| `R` | 可靠度 | `exp(-∫lambda(t)dt)`，代表機器能無故障運行至今的機率。 |
| `T*` | 最佳維護時機 | 理論上能使單位時間成本最小化的下一個維護時間間隔。 |
| `multiplier` | PHM損耗乘子 | 根據當前加工的作業特性，對基礎故障率的加速影響因子。 |
| `load` | 機器負載 | 單台機器的時間利用率。 |
| `norm_pdm_per_machine`| 單機PM次數 | 單台機器已執行的PM次數，經過標準化。 |

### 各網路的特徵視野 (Feature Scope for Each Network)

#### 1. 上層 Agent (經理) - `upper`
**職責**：決定進行「生產」還是「維護」。
**視野**：最廣闊，看到所有資訊，以做出最高層的戰略決策。

*   **全域特徵**:
    *   `norm_num_tardy_jobs`, `due_ratio`, `CRO`, `max_failure_risk`, `mean_machine_load`, `std_machine_load`, `diff_machine_load`, `norm_total_pdm`
*   **各機器特徵**:
    *   全部使用 (`age`, `lambda`, `R`, `T*`, `multiplier`, `load`, `norm_pdm_per_machine`)

#### 2. 生產專家 (下層) - `prod`
**職責**：選擇一個生產派工規則。
**視野**：專注於生產效率和排程相關的資訊，過濾掉了維護細節。

*   **全域特徵**:
    *   `norm_num_tardy_jobs`, `due_ratio`, `CRO`, `mean_machine_load`, `std_machine_load`, `diff_machine_load`
*   **各機器特徵**:
    *   只使用 `load`

#### 3. 維護專家 (下層) - `pm`
**職責**：選擇要對哪一台機器進行維護。
**視野**：專注於機器的健康狀況和維護成本，同時也關注延遲情況。

*   **全域特徵**:
    *   `max_failure_risk`, `norm_num_tardy_jobs`, `due_ratio`
*   **各機器特徵**:
    *   使用除了 `load` 以外的所有維護相關特徵 (`age`, `lambda`, `R`, `T*`, `multiplier`, `norm_pdm_per_machine`)
---
## 動作與獎勵函數設計 (Action & Reward Function Design)
...
2.  **生產專家 (`prod`)**:
    *   **動作**: 從 7 個預定義的派工規則中選擇一個。

| 動作 ID | 規則名稱 | 英文縮寫 | 描述 |
| :--- | :--- | :--- | :--- |
| 0 | 最長剩餘處理時間 | MRT | 優先處理剩餘總工時最長的作業。 |
| 1 | 最短剩餘處理時間 | LRT | 優先處理剩餘總工時最短的作業。 |
| 2 | 最短加工時間 | SPT | 優先處理下道工序加工時間最短的作業。 |
| 3 | 最長加工時間 | LPT | 優先處理下道工序加工時間最長的作業。 |
| 4 | 最早交付日期 | EDD | 優先處理交付日期最早的作業。 |
| 5 | 最小臨界比 | MCR | 優先處理 `(交付日期 - 當前時間) / 剩餘總工時` 比值最小的作業。 |
| 6 | 最大單位延遲成本 | MUTC | 優先處理單位時間延遲懲罰最高的作業。 |

3.  **維護專家 (`pm`)**:
    *   **動作**: 一個包含 `N` 個選項的離散選擇，`N` 是機器的總數。
    *   選擇 `k` (0 到 N-1) 代表決定對 `k+1` 號機器執行一次預防性維護。

### 獎勵函數 (Reward Function)

為了讓不同的 Agent 學會各自的職責，我們設計了不同的獎勵函數：

1.  **上層 Agent (`upper`) 和維護專家 (`pm`)**:
    *   **獎勵 `r_upper`, `r_pm`**: `-(新總成本)`
    *   **說明**: 這兩個 Agent 的決策影響深遠，因此它們的獎勵直接與**當前時刻的系統總成本** (`TTC + TBC + TMC`) 的負值掛鉤。系統總成本越低，它們獲得的獎勵就越高（一個更接近 0 的負數）。這驅使它們做出能讓系統長期保持在低成本狀態的決策。

2.  **生產專家 (`prod`)**:
    *   **獎勵 `r_prod`**: `-(ΔTTC + ΔTBC)`
    *   **說明**: 生產專家的決策是戰術性的，影響是局部的、立即的。因此，它的獎勵被設計為**成本的變化量 (Differential Reward)**。
        *   `ΔTTC` 是指執行它的動作後，TTC 成本相對於上一步的**變化量**。
        *   如果它的決策讓延遲成本和不平衡成本**下降**了，它就會獲得一個**正向**的獎勵。
        *   如果它的決策讓成本**上升**了，它就會獲得一個**負向**的獎勵。
    *   這種設計讓生產專家的學習訊號非常直接和清晰：它只需要專注於「我這個派工決定，是讓當前的排程變好了還是變壞了？」，而不需要考慮維護成本等與它無關的因素。
---
## 學習演算法：SAC 詳解 (Learning Algorithm: SAC Explained)

我們的 Agent 使用的學習演算法是「軟性演員-評論家 (Soft Actor-Critic, SAC)」，它是一種先進的深度強化學習演算法，以其高效率和穩定性著稱。

### 1. 核心架構：演員-評論家 (Actor-Critic)
*   **演員 (Actor)**: 策略網路，負責根據當前狀態**做出決策**（選擇一個動作）。在我們的架構中，`upper`, `prod`, `pm` 都是演員。
*   **評論家 (Critic)**: 價值網路，負責**評價演員的決策**。它會學習去預測，在某個狀態下採取某個動作，長期來看能獲得多少累積獎勵（Q 值）。它的評價會用來指導演員優化其決策。

### 2. 關鍵技巧 1：雙評論家網路 (Double-Critic Networks)
*   **問題**: 傳統的 Q-Learning 演算法有時會過度高估某些動作的價值，導致學習不穩定。
*   **SAC 的解法**: 我們的程式碼為每一個 Critic 都創建了兩個獨立的網路 (`critic1`, `critic2`)。在計算目標價值時，總是悲觀地選取兩者中**較小**的那個預測值。
*   **作用**: 這個技巧被稱為「Clipped Double Q-Learning」，它能有效抑制過度高估的問題，讓價值的學習過程更保守、更穩定。

### 3. 關鍵技巧 2：目標網路與軟更新 (Target Networks & Soft Update)
*   **問題**: 如果讓 Critic 網路去預測一個由自己（或一個與自己同步更新的網路）產生的、不斷變化的目標，學習過程會像「追逐自己的尾巴」一樣，非常不穩定。
*   **SAC 的解法**: 我們為 Critic 網路創建了兩個「目標網路」(`critic1_target`, `critic2_target`)。這兩個網路是主網路的**慢動作複製品**。
*   **作用**:
    *   在計算目標 Q 值時，我們使用這些**幾乎固定**的目標網路，從而為學習提供了一個穩定的錨點。
    *   每次主網路更新後，目標網路並非直接複製，而是透過「軟更新」的方式，向主網路的參數**緩慢地**靠攏。
        > `target_weights = τ * local_weights + (1 - τ) * target_weights`
    *   `τ (tau)` 是一個很小的值（例如 0.005），確保了目標網路的更新是平滑且延遲的。

### 4. SAC 的精髓：最大化熵與隨機策略 (Maximum Entropy & Stochastic Policy)
*   **傳統 RL 的目標**: `最大化累積獎勵`。
*   **SAC 的目標**: `最大化(累積獎勵 + α * 策略的熵)`。
*   **熵 (Entropy)**: 在這裡可以直觀地理解為 Agent 策略的「隨機性」或「不確定性」。
*   **意義**: 這個公式意味著，SAC 不僅獎勵那些能帶來高回報的行為，也**直接獎勵那些更具探索性的、更多樣化的行為**。`α (alpha)` 參數（熵溫度係數）就是用來平衡「追求回報」和「保持探索」這兩個目標的權重。
*   **結果**: 這使得 SAC 成為一個天生的「探索者」，能更有效地探索複雜的動作空間，避免過早地陷入局部最優解，這對於解決組合爆炸的排程問題至關重要。

### 5. 學習流程：`update` 方法詳解
`HRL_SAC` 類中的 `update` 方法整合了以上所有技巧，其流程如下：
1.  **抽樣**: 從 Replay Buffer 中隨機抽取一批 `(s, a, r, s')` 經驗。
2.  **計算目標**: 使用**目標網路** (`target_critics`) 來計算下一狀態 `s'` 的價值，並結合真實獎勵 `r` 和熵 `α*log(π)`，計算出一個穩定的「目標 Q 值」。
3.  **更新評論家**:
    *   讓兩個主評論家網路 (`critic1`, `critic2`) 預測當前狀態 `s` 和動作 `a` 的 Q 值。
    *   計算這兩個預測值與「目標 Q 值」之間的均方誤差 (MSE Loss)。
    *   透過反向傳播，更新兩個 Critic 網路的權重，讓它們的預測更接近目標。
4.  **更新演員**:
    *   更新 Actor 網路，使其更傾向於輸出那些能被 Critic 網路評為「高價值」的動作。同時，熵最大化的目標也會被納入 Actor 的損失函數中，以鼓勵探索。
5.  **軟更新目標網路**:
    *   最後，對兩個 Target Critic 網路的權重進行一次微小的、平滑的軟更新，讓它們慢慢追趕上主網路的進度。
